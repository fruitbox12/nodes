...
class Pinata {
    ...
    async run(nodeData) {
        ...
        const cidVersion = inputParametersData.cidVersion;
        const wrapWithDirectory = inputParametersData.wrapWithDirectory;
        const name = inputParametersData.name;
        const keyvalues = inputParametersData.keyvalues;
        const hostNodes = inputParametersData.hostNodes;
        const returnData = [];
        let responseData;
        let url = '';
        ...
        function getOptionsAndMetadata() {
            // Metadata
            const pinataMetadataObj = {};
            if (name)
                pinataMetadataObj.name = name;
            if (keyvalues) {
            ...
            // Options
            const pinataOptionsObj = {};
            if (cidVersion)
                pinataOptionsObj.cidVersion = parseInt(cidVersion);
            if (wrapWithDirectory)
                pinataOptionsObj.wrapWithDirectory = wrapWithDirectory;
            if (hostNodes) {
                const hostNodesArray = JSON.parse(hostNodes.replace(/\s/g, ''));
                if (hostNodesArray.length)
                    pinataOptionsObj.hostNodes = hostNodesArray;
            }
        ...
        try {
            if (operation === 'json') {
                // Json Content
                const jsonContent_str = actionData.jsonContent;
                if (wrapWithDirectory) {
                ...
                // Json Content Array
                const jsonContentArray_str = actionData.jsonContentArray;
                ...
                // Base64 Content
                const base64Content_str = actionData.base64Content;
                //base64Content_str = data:image/png;base64,<base64_string>
                url = 'https://api.pinata.cloud/pinning/pinFileToIPFS';
                const splitDataURI = base64Content_str.split(',');
                const bf = Buffer.from(splitDataURI.pop() || '', 'base64');
                let ext = 'png';
                const extension = (_a = splitDataURI.pop()) === null || _a === void 0 ? void 0 : _a.split(';')[0].split('/')[1];
                if (extension)
                    ext;
                else
                    ext = getExtension(base64Content_str);
                ...
                const base64ContentArray_str = actionData.base64ContentArray;
                ...
                for (let i = 0; i < base64ContentArray.length; i += 1) {
                    ...
                    const bf = Buffer.from(splitDataURI.pop() || '', 'base64');
                    let ext = 'png';
                    const extension = (_b = splitDataURI.pop()) === null || _b === void 0 ? void 0 : _b.split(';')[0].split('/')[1];
                    if (extension)
                        ext;
                    else
                        ext = getExtension(base64ContentArray[i]);
                ...
                url = 'https://api.pinata.cloud/pinning/pinFileToIPFS';
                const fileBase64 = actionData.fileContent;
                ...
                const hashToPin = actionData.hashToPin;
                ...
                method = 'GET';
                const status = inputParametersData.status;
                ...
                method = 'PUT';
                const ipfsPinHash = actionData.ipfsPinHash;
                ...
                const pinToDelete = actionData.pinToDelete;
